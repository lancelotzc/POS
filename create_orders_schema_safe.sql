...
CREATE TABLE order_items (
    ...
    modifiers JSONB DEFAULT '[]'::jsonb, -- 紀錄加料與客製化選項
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- 4. 建立絕對嚴格的 RLS 政策
-- 先移除可能存在的舊政策
DROP POLICY IF EXISTS "Strict Policy for Orders" ON orders;
DROP POLICY IF EXISTS "Strict Policy for Order Items" ON order_items;
